...
def classify(coding_seq, non_coding_seq, tmp_file_location, raw_read, name):
    ...
    .format(circ,tmp_file_location+'/'+ribo_name+'_junction_result',RCRJ),shell=True)
    ...
    '''.format(coding_seq, non_coding_seq, tmp, RCRJ, tmp+'.dat', RCRJ, tmp_file_location+'/'+ribo_name+'_translated_circ.fa')
    print(r_script)
    f=open('r_script.r','w')
    f.write(r_script)
    f.close()
    ...
    subprocess.call('''sed -i 's/()//g' {}'''.format(tmp_file_location+'/'+ribo_name+'_translated_circ.fa'),shell=True)
...
def find_longest(tmp_file_location, raw_read, result_file_location, number):
    ...
    SeqIO.write(seq_list, '{}'.format(result_file_location+'/'+'result_pep_{}.fa'.format(number)), 'fasta')
